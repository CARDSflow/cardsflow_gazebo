...
public:
	GeodesicAlgorithmExact(geodesic::Mesh* mesh) :
			GeodesicAlgorithmBase(mesh), m_memory_allocator(
					mesh->edges().size(), mesh->edges().size()), m_edge_interval_lists(
					mesh->edges().size())
	{
		m_type = EXACT;
...
private:
	...
	void update_list_and_queue(list_pointer list, IntervalWithStop* candidates, //up to two candidates
	...
	void construct_propagated_intervals(bool invert, edge_pointer edge,
	...
	double compute_positive_intersection(double start, double pseudo_x,
			double pseudo_y, double sin_alpha, double cos_alpha); //used in construct_propagated_intervals

	unsigned intersect_intervals(interval_pointer zero, IntervalWithStop* one); //intersecting two intervals with up to three intervals in the end
	...
	{
		OLD, NEW
	}; //used for interval intersection
	MapType map[5];
	double start[6];
...
inline long GeodesicAlgorithmExact::visible_from_source(SurfacePoint& point) //negative if not visible
{
	...
	{
		throw sgal::Exception("Geodesic: UNDEFINED POINT");
	}
	...
	//assert(0);
	throw sgal::Exception("Geodesic not visible from source");
	return 0;
}
		...
		double cos_alpha)
{
	...
	if (pseudo_y >= 0)
	{
		throw sgal::Exception("Geodesic: pseudo_y >= 0");
	}
		...
		MeshElementBase* p, std::vector< edge_pointer >& storage)
{
	...
	if (p->type() == UNDEFINED_POINT)
	{
		throw sgal::Exception(
				"Geodesic::list_edges_visible_from_source Undefined Point");
	}
...
inline bool GeodesicAlgorithmExact::erase_from_queue(interval_pointer p)
{
	if (p->min() < GEODESIC_INF / 10.0) // && p->min >= queue->begin()->first)
	{
		...
		if (m_queue.count(p) > 1)
		{
			throw sgal::Exception(
					"GeodesicAlgorithmExact::erase_from_queue - set not unique");
		}
		//the set is unique
...
inline unsigned GeodesicAlgorithmExact::intersect_intervals(
		interval_pointer zero, IntervalWithStop* one) //intersecting two intervals with up to three intervals in the end
{
	//assert(zero->edge()->id() == one->edge()->id());

	if (zero->edge()->id() != one->edge()->id())
	{
		throw sgal::Exception("GeodesicAlgorithmExact::intersect_intervals");
	}
	...
	double const local_epsilon = SMALLEST_INTERVAL_RATIO
			* one->edge()->length();
...
inline void GeodesicAlgorithmExact::propagate(
		std::vector< SurfacePoint >& sources, double max_propagation_distance, //propagation algorithm stops after reaching the certain distance from the source
		std::vector< SurfacePoint >* stop_points)
{
	...
	while (!m_queue.empty())
	{
		m_queue_max_size = m_queue.size() > 0 ? m_queue.size() : 0;
		...
		{
			throw sgal::Exception("GeodesicAlgorithmExact::propagate");
		}
		...
		for (unsigned i = 0; i < edge->adjacent_faces().size(); ++i) //two possible faces to propagate
		{
			...
			{
				if ((i == 0
						&& min_interval->direction() == Interval::FROM_FACE_0)
						|| (i == 1
								&& min_interval->direction()
			...
			unsigned num_propagated = compute_propagated_parameters(
					...
					min_interval->start(), min_interval->stop(), //start/end of the interval
					...
					last_interval, turn_left, turn_right, candidates); //if it is the last interval on the edge
			bool propagate_to_right = true;

			if (num_propagated)
			{
				if (candidates[num_propagated - 1].stop()
						!= next_edge->length())
				{
				...
				construct_propagated_intervals(
						...
						next_edge, face, candidates, num_propagated,
						min_interval);
			...
			if (propagate_to_right)
			{
				...
				num_propagated = compute_propagated_parameters(
						...
						first_interval, turn_right, turn_left, candidates); //if it is the last interval on the edge

				if (num_propagated)
				{
					bool const invert = next_edge->v0()->id()
							!= edge->v1()->id(); //if the origins coinside, do not invert intervals
	...
	clock_t stop = clock();
	m_time_consumed = (static_cast< double >(stop)
		...
		unsigned num_candidates)
{
	...
	if (num_candidates > 2)
	{
		throw sgal::Exception("GeodesicAlgorithmExact::update_list_and_queue");
	}
	...
	if (list->first() == NULL)
	{
		...
		else
		{
			...
			{
				throw sgal::Exception(
						"GeodesicAlgorithmExact::update_list_and_queue");
			}
			...
			for (unsigned j = 0; j < N; ++j) //find "min" and add the intervals to the queue
			{
				...
				else
				{
					...
					current_interval->compute_min_distance(
							current_interval->stop()); //compute minimal distance
		...
		interval_pointer source_interval) //up to two candidates
{
	...
	Interval::DirectionType direction =
			edge->adjacent_faces()[0]->id() == face->id() ?
					Interval::FROM_FACE_0 : Interval::FROM_FACE_1;
		...
		double& best_interval_position, unsigned& best_source_index)
{
	...
	if (point.type() == UNDEFINED_POINT)
	{
		throw sgal::Exception("GeodesicAlgorithmExact::best_first_interval");
	}
		...
		std::vector< SurfacePoint >& path)
{
	...
	interval_pointer best_interval = best_first_interval(destination,
			best_total_distance, best_interval_position, source_index);
	...
	if (best_interval) //if we did not hit the face source immediately
	{
		...
		while (visible_from_source(path.back()) < 0) //while this point is not in the direct visibility of some source (if we are inside the FACE, we obviously hit the source)
		{
			...
			{
				throw sgal::Exception("GeodesicAlgorithmExact::trace_back");
			}
			...
			/*if (i > 2000)
			 {
			 std::cout << path[path.size() - 1].x() << ", "
			 << path[path.size() - 1].y() << ", "
			 << path[path.size() - 1].z() << std::endl;
			 }*/
		}
	}
...
inline void GeodesicAlgorithmExact::print_statistics()
{
	...
	double intervals_per_edge = (double) interval_counter
